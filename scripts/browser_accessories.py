...
def candidate_links(html, base_url):
    ...
    for a in soup.find_all('a',href=True):
        ...
        if not is_product_href(href) or href in seen:continue
        seen.add(href)
        parent=a
        for _ in range(3):
            if parent.parent is None:break
            parent=parent.parent
        text=' '.join(parent.stripped_strings)[:1500]
        if not text:text=a.get_text(' ',strip=True)
        img=None
        imgtag=parent.find('img')
        if imgtag:
            img=imgtag.get('src') or imgtag.get('data-src') or imgag if False else None
            if not img:
                img=imgtag.get('data-lazy-src') or imgag if False else None
...
def best_candidate(name,cands):
    ...
    for c in cands:
        ...
        # URLs often contain cleaner product slugs than card text.
...
def inspect_product(page,url,name,old_price):
    ...
    return {
        'url':final,'stock':stock,'price':picked,
        'img':extract_image(soup,final,name)
    }
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # Cache category/search pages because many accessories share one source.
        cache={}
        kept=[]
        for acc in accessories:
            url=acc.get('url','')
            direct=is_product_href(url)
            candidate_img=None
            if not direct and url:
                ...
                score,c=best_candidate(acc.get('name',''),cache[url])
                if c:
            ...
            if direct:
                ...
                try:
                    ...
                    acc['scan_status']='browser_ok'
                    acc['direct_product_url']=True
                    ...
                    acc['url']=result.get('url') or url
                    verified+=1
                except Exception as e:
                    acc['last_checked']=datetime.now(timezone.utc).isoformat(timespec='seconds')
                    acc['scan_status']='browser_error'
                    acc['browser_error']=str(e)[:160]
