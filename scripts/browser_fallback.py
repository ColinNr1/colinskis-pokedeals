...
from update_catalog import (
    ...
    extract_jsonld_product, PRICE_RE, parse_price, bad_image,
    is_direct_product_url, match_ratio, IN_WORDS, OUT_WORDS
)
...
def clean_slug(url):
    path=urlparse(url or '').path.lower().replace('-',' ').replace('_',' ')
...
def match_evidence(soup, final_url, name, text):
    ...
    slug=clean_slug(final_url)
    product=extract_jsonld_product(soup,name)
    json_name=product.get('name','') if isinstance(product,dict) else ''

    scores={
        'title':match_ratio(name,title),
        'h1':match_ratio(name,h1),
        'slug':match_ratio(name,slug),
        'jsonld':match_ratio(name,json_name),
    }
    near=find_near_text(text,name)
    scores['body']=1.0 if near else 0.0

    # High confidence from one strong structured signal, or two independent medium signals.
    strong=max(scores.values())>=.62
    medium=sum(1 for v in scores.values() if v>=.42)>=2
    accepted=strong or medium
    return accepted, near, scores


def inspect_html(html, final_url, name, old_price):
    soup=BeautifulSoup(html,'html.parser')
    text=' '.join(soup.stripped_strings)
    accepted,near,scores=match_evidence(soup,final_url,name,text)
    if not accepted:
        return {
            'status':'browser_not_matched',
            'image':extract_image(soup,final_url,name),
            'match_scores':scores
        }

    scoped=near or text[:12000]
    lower=scoped.lower()
    stock='unknown'
    ...
    return {
        ...
        'image':extract_image(soup,final_url,name),'match_scores':scores
    }
...
def main():
        ...
        for group in ('products','preorders'):
            for item in data.get(group,[]):
                ...
                for listing in item.get('listings') or []:
                    ...
                    try:
                        page.goto(listing['url'],wait_until='domcontentloaded',timeout=22000)
    ...
    data.setdefault('scan_summary',{})['browser_fallback']={
        ...
        'product_bing_images_remaining':bing,
        'matcher_version':'2.0-multisignal'
    }
    ...
    print(f'Browser fallback attempted {attempted}; recovered {recovered}; bing={bing}; statuses={statuses}')
