...
def match_ratio(a,b):
    ...
    if not toks:
        return 0
    return sum(1 for t in toks if t in nb)/len(toks)
...
def is_direct_product_url(url):
    ...
    return (
        '/product/' in p or '/product-page/' in p or '/products/' in p
...
def find_near_text(text,name):
    ...
    for a in sorted(toks,key=len,reverse=True):
        ...
        while True:
            ...
            if sum(1 for t in toks if t in w)/max(1,len(toks))>=.45:
                return w
            start=i+len(a)
    return None
...
def extract_image(soup, final_url, product_name):
    ...
    if product:
        ...
        if isinstance(image,str) and image.startswith(('http://','https://','/')):
            return urljoin(final_url,image)
...
def inspect_listing(listing,product_name):
    ...
    title_match=match_ratio(product_name,page_title)
    if not near and title_match<.35:
        return {'status':'not_matched','final_url':final,'image':extract_image(soup,final,product_name)}

    scoped=near or norm(text[:6000])
    lower=scoped.lower()
    stock='unknown'
    ...
    return {
        'status':'ok','stock':stock,'price':price,'final_url':final,
        'direct_url':True,'image':extract_image(soup,final,product_name)
    }


def bad_image(url):
    u=(url or '').lower()
    return (not u) or 'bing.net' in u or u.startswith('data:image/svg+xml')
    ...
    for item in items:
        ...
        for listing in item.get(key) or []:
            ...
            if res.get('stock')=='out_of_stock':
                changes.append(f"{item['name']} / {listing.get('store')}: removed (out of stock)")
                continue
            kept.append(listing)
            time.sleep(.12)
        item[key]=kept
        if best_image and bad_image(item.get('img')):
            item['img']=best_image
            changes.append(f"{item['name']}: replaced placeholder/search image with vendor product image")
        ...
        for item in data.get(group,[]):
            for listing in item.get('listings') or []:
                if not is_direct_product_url(listing.get('url','')):
                    invalid.append(f"{group}: {item.get('name')} / {listing.get('store')} -> {listing.get('url')}")
...
def main():
    ...
    data['scan_summary']={'checked_listings':total,'changes':all_changes[:150],'scanner_version':'1.2-browser-jsonld-images','statuses':statuses}
