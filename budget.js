/* Colinskis PokeDeals — Budget Allocator V3 (single-store aware) */
(()=>{
...
function verifiedListings(p){
  return (p.listings||[]).filter(x=>+x.price>0 && (x.live_verified===true || ['ok','browser_ok'].includes(x.scan_status)));
}
...
function applyPackBadges(products){document.querySelectorAll('.card').forEach(card=>{const h=card.querySelector('h3');if(!h)return;const p=products.find(x=>x.name===h.textContent.trim());if(!p)return;const pc=packs(p),ls=verifiedListings(p),bp=ls.length?Math.min(...ls.map(x=>+x.price)):Infinity;if(!pc||!isFinite(bp)||card.querySelector('.packintel'))return;const d=document.createElement('div');d.className='packintel';d.innerHTML=`<b>${pc} booster${pc===1?'':'s'}</b><span>from ${money(bp/pc)} / booster</span>`; const body=card.querySelector('.body')||card;body.insertBefore(d,body.querySelector('.prices')||null);});}
